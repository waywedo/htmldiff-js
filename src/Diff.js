...
class HtmlDiff {
        ...
        while (words.length) {
            ...
            if (nonTags.length !== 0) {
                ...
                if (specialCaseTagInjectionIsbefore) {
                    this.content.push(
                        specialCaseTagInjection + this.extractConsecutiveWords(words, Utils.isTag).join("")
                    );
                } else {
                    this.content.push(
                        this.extractConsecutiveWords(words, Utils.isTag).join("") + specialCaseTagInjection
                    );
                }
            }
        }
    ...
    operations() {
        ...
        while (!(matchResult = matchesWithoutOrphans.next()).done) {
            ...
            if (match.length !== 0) {
                operations.push(
                    new Operation(Action.equal, match.startInOld, match.endInOld, match.startInNew, match.endInNew)
                );
            }
